...
All commands start with `zanken-`. Prefixes indicate purpose.

The authoritative command group list lives in `bin/zanken` in `GROUP_DESCRIPTIONS`. Keep `GROUP_DESCRIPTIONS` updated when adding a new command prefix.
...
Commands in `bin/` can declare CLI metadata in comments near the top of the file. `bin/zanken` scans the first 80 lines, and tests expect command metadata to remain valid.

Supported metadata keys:

- `# zanken:summary=...` - short help text
- `# zanken:group=...` - command group when it differs from the filename-derived prefix
- `# zanken:name=...` - command name within the group
- `# zanken:args=...` - usage arguments
- `# zanken:examples=...` - examples separated with ` | `
- `# zanken:alias=...` / `# zanken:aliases=...` - alternate routes
- `# zanken:hidden=true` - hide from default command listings
- `# zanken:requires-sudo=true` - mark commands that require sudo
...
# zanken:args=[smart|region|windows|fullscreen] [slurp|copy]
...
- use `$OMARCHY_INSTALL` and `$OMARCHY_PATH` instead of hard-coded Zanken paths
...
- `zanken-cmd-missing` / `zanken-cmd-present` - check for commands
- `zanken-pkg-missing` / `zanken-pkg-present` - check for packages
- `zanken-pkg-add` - install packages (handles both pacman and AUR)
- `zanken-pkg-drop` - remove packages; use this instead of raw `pacman -R*`
- `zanken-notification-send` - send desktop notifications; do not call `notify-send` directly
- `zanken-hw-asus-rog` - detect ASUS ROG hardware (and similar `hw-*` commands)
...
When making visual changes, such as Waybar styles or desktop appearance, always take and analyze a screenshot after applying the change to verify the result. Use `zanken capture screenshot fullscreen save` for fullscreen screenshots.
...
```bash
zanken-refresh-config hypr/hyprlock.conf
```

This copies `~/.local/share/zanken/config/hypr/hyprlock.conf` to `~/.config/hypr/hyprlock.conf`.

# Migrations

To create a new migration, run `zanken-dev-add-migration --no-edit`. This creates a migration file named after the unix timestamp of the last commit.

New migration format:
- File permissions must be `0644` (`-rw-r--r--`); migrations are sourced, not executed directly
- No shebang line
- Start with an `echo` describing what the migration does
- Use `$OMARCHY_PATH` to reference the zanken directory
- Prefer helper commands such as `zanken-cmd-present`, `zanken-cmd-missing`, `zanken-pkg-present`, and `zanken-pkg-missing`

Some older migrations predate these rules. Do not copy older migrations that start with shebangs, omit the leading `echo`, or hard-code `~/.local/share/zanken`.
...
if zanken-cmd-missing fprintd-list || ! fprintd-list "$USER" 2>/dev/null | grep -q "finger"; then
