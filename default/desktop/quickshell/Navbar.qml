...
Item {
    ...
    signal paletteOpenCategoryRequested(string cat)
    signal paletteOpenQuickTileRequested(string key)
    ...
    readonly property string icoMicOn:  String.fromCodePoint(0xf130)    // fa-microphone
    readonly property string icoMicOff: String.fromCodePoint(0xf131)    // fa-microphone-slash
    ...
    readonly property string icoStop:    String.fromCodePoint(0xf04d)
    readonly property string icoPrev:    String.fromCodePoint(0xf04a)
    readonly property string icoNext:    String.fromCodePoint(0xf04e)
    readonly property string icoRepeat:  String.fromCodePoint(0xf01e)
    readonly property string icoShuffle: String.fromCodePoint(0xf074)
    readonly property string icoCoffee:  String.fromCodePoint(0xf0f4)
    ...
    property Item bluetoothAnchorItem: null
    property Item audioAnchorItem:    null
    property Item musicAnchorItem:    null
    property Item batteryAnchorItem:  null
    ...
    readonly property int trayItemCount: SystemTray.items ? SystemTray.items.values.length : 0
    ...
    NotificationServer {
        ...
        onNotification: function(n) {
            n.tracked = true;
            const item = { appName: n.appName, summary: n.summary, body: n.body, time: Date.now(), notif: n };
            root.cardItems = root.cardItems.concat([item]);
    ...
    FileView {
        id: notifStore
        path: root.notifStorePath
        blockLoading: true
    }

    FileView {
        id: barStateStore
        path: StandardPaths.writableLocation(StandardPaths.CacheLocation) + "/qs-bar-state.json"
        blockLoading: true
    }
    ...
    readonly property string notifStorePath: StandardPaths.writableLocation(StandardPaths.CacheLocation) + "/qs-notifications.json"
    readonly property string clipStorePath:  StandardPaths.writableLocation(StandardPaths.CacheLocation) + "/qs-clipboard.json"
    ...
    Connections {
        ...
        function onClipboardTextChanged() {
            ...
            const filtered = root.clipHistory.filter(function(c) { return c.content !== t; });
            root.clipHistory = [{content: t, time: Date.now()}].concat(filtered).slice(0, 50);
            clipStore.setText(JSON.stringify(root.clipHistory));
        }
    }
    property var cardItems: []
    readonly property int notificationCount: root.cardItems.length
    ...
    onDoNotDisturbChanged: root.saveBarState()



    function saveNotifications() {
        const data = root.cardItems.map(function(c) {
            return { appName: c.appName, summary: c.summary, body: c.body, time: c.time };
        });
        notifStore.setText(JSON.stringify(data));
    }
    ...
    function openNotifications() {
        ...
        root.notificationsVisible = true;
    }
    ...
    function openTray() {
        ...
        root.closeAllCards();
        if (root.trayAnchorItem) root.anchorPopupTo(root.trayAnchorItem);
        root.trayVisible = true;
    }
    ...
    function removeClipItem(index) {
        root.clipHistory = root.clipHistory.filter(function(c, i) { return i !== index; });
        clipStore.setText(JSON.stringify(root.clipHistory));
    }

    property bool idleInhibit:      false

    function toggleIdleInhibit() { root.idleInhibit = !root.idleInhibit; }
    ...
    Timer {
        ...
        repeat: false
        onTriggered: {
            // Silent re-probe after connect/disconnect — no wifiScanning=true, no SCANNING animation
        ...
        onRunningChanged: {
            if (!running) {
                if (exitCode === 0) {
                    ...
                    root.wifiConnectError = root.wifiConnectingSSID;
                    root.wifiConnectingSSID = "";
                    wifiConnectErrorTimer.restart();
                }
            }
        }
    }
    Timer {
        ...
        interval: 3000
        repeat: false
        onTriggered: { root.wifiConnectError = ""; }
    }
    ...
    function btDisconnect(mac) {
        if (!mac) return;
        root.run("bluetoothctl disconnect " + mac);
        btPostActionTimer.restart();
    }
    function btTogglePower() {
        root.btPowered = !root.btPowered;
        root.run("bluetoothctl power " + (root.btPowered ? "on" : "off"));
        btPostActionTimer.restart();
    }
    ...
    property var    audioStreams: []        // [{index, name, vol, muted}, ...]
    property var    audioInputSources: []  // [{id, srcName, name, vol, muted, isDefault}]
    property var    audioInputStreams: []  // [{index, name, vol, muted}]
    ...
    function refreshAudioStreams() {
        audioStreamsProbe.running = false;
        audioStreamsProbe.running = true;
    }
    function setStreamVolume(index, pct) {
        root.run("pactl set-sink-input-volume " + index + " " + Math.max(0, Math.min(100, pct)) + "%");
        audioStreamsRefreshTimer.restart();
    }
    function toggleStreamMute(index) {
        root.run("pactl set-sink-input-mute " + index + " toggle");
        audioStreamsRefreshTimer.restart();
    }
    function refreshAudioInputs() {
        audioInputSourcesProbe.running = false;
        audioInputSourcesProbe.running = true;
        audioInputStreamsProbe.running = false;
        audioInputStreamsProbe.running = true;
    }
    function setDefaultSource(srcName) {
        if (!srcName) return;
        root.run("pactl set-default-source " + JSON.stringify(srcName));
        audioInputSourcesRefreshTimer.restart();
    }
    function setSourceVolume(srcName, pct) {
        if (!srcName) return;
        root.run("pactl set-source-volume " + JSON.stringify(srcName) + " " + Math.max(0, Math.min(100, pct)) + "%");
        audioInputSourcesRefreshTimer.restart();
    }
    function toggleSourceMute(srcName) {
        if (!srcName) return;
        root.run("pactl set-source-mute " + JSON.stringify(srcName) + " toggle");
        audioInputSourcesRefreshTimer.restart();
    }
    function setInputStreamVolume(index, pct) {
        root.run("pactl set-source-output-volume " + index + " " + Math.max(0, Math.min(100, pct)) + "%");
        audioInputStreamsRefreshTimer.restart();
    }
    function toggleInputStreamMute(index) {
        root.run("pactl set-source-output-mute " + index + " toggle");
        audioInputStreamsRefreshTimer.restart();
    }
    ...
    // hyprsunset is Hyprland-only; color temp/gamma sliders are disabled on niri.
    // Brightness via brightnessctl works on any compositor.
    ...
    function runSunset(verb) { /* no-op on niri */ }

    function setWarmth(k) {
        k = Math.max(1000, Math.min(6500, Math.round(k / 50) * 50));
        root.warmthK = k;
        // color temperature not supported on niri
    }
    ...
    function setGamma(pct) {
        ...
        root.gammaPct = pct;
        // gamma not supported on niri
    }
    function applyPreset(p) {
        root.warmthK = p.warmth;
        root.gammaPct = p.gamma;
        root.brightnessPct = p.bright;
        root.run("brightnessctl set " + p.bright + "%");
    }
    function blankScreen() {
        root.run("sleep 0.25 && niri msg action power-off-monitors");
    ...
    function resetDisplay() {
        ...
        root.run("brightnessctl set 100%");
    }
    ...
    Process {
        ...
        running: false
        command: ["bash", "-lc",
            "m=$(niri msg --json outputs 2>/dev/null"
            + " | jq -r '.[0] | [.name,(\"\\(.logical.width)x\\(.logical.height)\"),(.current_mode.refresh_rate / 1000 | tostring),(.logical.scale | tostring)] | join(\"|\")' 2>/dev/null);"
    ...
    //   ~/.config/zanken/weather/location
    ...
    readonly property string weatherLocationPath: Quickshell.env("HOME") + "/.config/zanken/weather/location"
    ...
    function openBattery() {
        root.closeAllCards();
        if (root.batteryAnchorItem) root.anchorPopupTo(root.batteryAnchorItem);
        root.batteryVisible = true;
    ...
    function openBluetooth() {
        root.closeAllCards();
        if (root.bluetoothAnchorItem) root.anchorPopupTo(root.bluetoothAnchorItem);
    ...
    Process {
        ...
        command: ["bash", "-lc",
            ...
            + "printf '%d|%d|%d|%s|%s|%s|%s|%s|%d|%d' "
            + "  \"$cpu\" \"$mem\" \"$bat\" \"$bst\" "
            + "  \"$(date +%H)\" \"$(date +%M)\" \"$(date +%d)\" \"$(date +%b | tr a-z A-Z)\" \"$pwr\" \"$disk\""]
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                if (p.length >= 9) {
                    ...
                    if (p.length >= 10) root.diskVal = parseInt(p[9]) || 0;
                }
            }
        }
    }
    ...
    Process {
        ...
        command: ["bash", "-lc",
            ...
            + "act=$(printf '%s' \"$ws\" | jq -r '[.[] | select(.is_focused)] | .[0].idx // 1' 2>/dev/null || echo 1); "
            + "ids=$(printf '%s' \"$ws\" | jq -r '[.[].idx] | join(\",\")' 2>/dev/null || echo 1); "
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                if (p.length === 2) {
                    ...
                    root.existingWs = [...new Set([...have, 1])].sort((a,b) => a-b).slice(0, 9);
                }
            }
        }
    }
    ...
    Process {
        ...
        command: ["bash", "-lc",
            ...
            + " if [ \"$p\" = 'yes' ]; then echo on; else echo off; fi"]
        stdout: StdioCollector {
            onStreamFinished: {
    ...
    Process {
        ...
        command: ["bash", "-lc",
            ...
            + "   [ -z \"$info\" ] && continue;"
            + "   name=$(printf '%s' \"$info\" | awk -F': ' '/Name:/{print $2; exit}' | sed 's/[[:space:]]*$//');"
            + "   conn=$(printf '%s' \"$info\" | awk '/Connected: yes/{print 1; exit}');"
            + "   paired=$(printf '%s' \"$info\" | awk '/Paired: yes/{print 1; exit}');"
            + "   trusted=$(printf '%s' \"$info\" | awk '/Trusted: yes/{print 1; exit}');"
            + "   printf '%s\\t%s\\t%s\\t%s\\t%s\\n' \"$mac\" \"${name:-$mac}\" \"${conn:-0}\" \"${paired:-0}\" \"${trusted:-0}\";"
            + " done"]
        stdout: StdioCollector {
            onStreamFinished: {
    ...
    // ---------- Wi-Fi known-networks probe ----------
    // Populated at bar startup, every 30s, on openWifi(), and after forgetWifi().
    // Uses nmcli (NetworkManager) since NM owns wlan0 and holds saved credentials.
    Process {
        id: wifiKnownProbe
        running: false
        command: ["bash", "-lc",
            ...
            + " | sed 's/:802-11-wireless$//; s/\\\\:/:/g'"]
        stdout: StdioCollector {
            onStreamFinished: {
                root.wifiKnownSsids = this.text.trim().split("\n").filter(s => s.length > 0);
            }
        }
    }
    Timer {
        id: wifiKnownRefreshTimer
        interval: 30000
        repeat: true
        running: true
        onTriggered: {
    ...
    // ---------- Audio streams probe ----------
    // pactl list sink-inputs → per-app playback stream list.
    // Each output line: index\tname\tvol\tmuted
    Process {
        id: audioStreamsProbe
        running: false
        command: ["bash", "-lc",
            "pactl list sink-inputs 2>/dev/null | awk '"
            + "BEGIN{idx=0;name=\"\";vol=0;muted=0}"
            + "/^Sink Input #/{if(idx>0)printf \"%s\\t%s\\t%s\\t%s\\n\",idx,name,vol,muted;idx=substr($0,index($0,\"#\")+1)+0;name=\"\";vol=0;muted=0}"
            + "/^\\tMute: /{muted=($2==\"yes\")?1:0}"
            + "/^\\tVolume: /{match($0,/[0-9]+%/);vol=int(substr($0,RSTART,RLENGTH-1))}"
            + "/application\\.name =/{n=$0;sub(/.*= /,\"\",n);sub(/^\"/,\"\",n);sub(/\"$/,\"\",n);name=n}"
            + "END{if(idx>0)printf \"%s\\t%s\\t%s\\t%s\\n\",idx,name,vol,muted}'"
        ]
        stdout: StdioCollector {
            onStreamFinished: {
                const lines = this.text.trim().split("\n").filter(l => l.length > 0);
                root.audioStreams = lines.map(l => {
                    const f = l.split("\t");
                    return { index: parseInt(f[0]) || 0, name: f[1] || "Unknown", vol: parseInt(f[2]) || 0, muted: f[3] === "1" };
                });
            }
        }
    }
    Timer {
        id: audioStreamsRefreshTimer
        interval: 400
        repeat: false
        onTriggered: { audioStreamsProbe.running = false; audioStreamsProbe.running = true; }
    }

    // ---------- Audio input sources probe ----------
    // pactl list sources → mic/capture devices, skipping .monitor (loopback) sources.
    // Each output line: id\tsrcName\tfriendlyName\tvol\tmuted\tisDefault
    Process {
        id: audioInputSourcesProbe
        running: false
        command: ["bash", "-lc",
            "DEF=$(pactl info 2>/dev/null | awk '/Default Source:/{print $NF}');"
            + " pactl list sources 2>/dev/null | awk -v def=\"$DEF\" '"
            + "BEGIN{idx=0;sn=\"\";name=\"\";vol=0;muted=0;isdef=0;skip=0}"
            + "/^Source #/{if(idx>0&&!skip)printf \"%s\\t%s\\t%s\\t%s\\t%s\\t%s\\n\",idx,sn,name,vol,muted,isdef;idx=substr($0,index($0,\"#\")+1)+0;sn=\"\";name=\"\";vol=0;muted=0;isdef=0;skip=0}"
            + "/^\\tName: /{sn=$NF;if(sn~/\\.monitor$/)skip=1;if(sn==def)isdef=1}"
            + "/^\\tMute: /{muted=($2==\"yes\")?1:0}"
            + "/^\\tVolume: /{match($0,/[0-9]+%/);vol=int(substr($0,RSTART,RLENGTH-1))}"
            + "/^\\tDescription: /{n=$0;sub(/^\\tDescription: /,\"\",n);name=n}"
            + "END{if(idx>0&&!skip)printf \"%s\\t%s\\t%s\\t%s\\t%s\\t%s\\n\",idx,sn,name,vol,muted,isdef}'"
        ]
        stdout: StdioCollector {
            onStreamFinished: {
                const lines = this.text.trim().split("\n").filter(l => l.length > 0);
                root.audioInputSources = lines.map(l => {
                    const f = l.split("\t");
                    return { id: parseInt(f[0]) || 0, srcName: f[1] || "", name: f[2] || "Unknown", vol: parseInt(f[3]) || 0, muted: f[4] === "1", isDefault: f[5] === "1" };
                });
            }
        }
    }
    Timer {
        id: audioInputSourcesRefreshTimer
        interval: 400
        repeat: false
        onTriggered: { audioInputSourcesProbe.running = false; audioInputSourcesProbe.running = true; }
    }

    // Watches for external mic mute changes (e.g. XF86AudioMicMute keybind → wpctl).
    // pactl subscribe emits "Event 'change' on source #N" on any source state change.
    Process {
        id: pactlSourceWatcher
        running: true
        command: ["pactl", "subscribe"]
        onRunningChanged: if (!running) pactlSourceRestartTimer.start()
        stdout: SplitParser {
            splitMarker: "\n"
    ...
    Timer {
        id: pactlSourceRestartTimer
        interval: 2000
        repeat: false
        onTriggered: pactlSourceWatcher.running = true
    }

    // ---------- Audio input streams probe ----------
    // pactl list source-outputs → per-app capture streams (e.g. OBS, Discord).
    // Each output line: index\tname\tvol\tmuted
    Process {
        id: audioInputStreamsProbe
        running: false
        command: ["bash", "-lc",
            "pactl list source-outputs 2>/dev/null | awk '"
            + "BEGIN{idx=0;name=\"\";vol=0;muted=0}"
            + "/^Source Output #/{if(idx>0)printf \"%s\\t%s\\t%s\\t%s\\n\",idx,name,vol,muted;idx=substr($0,index($0,\"#\")+1)+0;name=\"\";vol=0;muted=0}"
            + "/^\\tMute: /{muted=($2==\"yes\")?1:0}"
            + "/^\\tVolume: /{match($0,/[0-9]+%/);vol=int(substr($0,RSTART,RLENGTH-1))}"
            + "/application\\.name =/{n=$0;sub(/.*= /,\"\",n);sub(/^\"/,\"\",n);sub(/\"$/,\"\",n);name=n}"
            + "END{if(idx>0)printf \"%s\\t%s\\t%s\\t%s\\n\",idx,name,vol,muted}'"
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                const lines = this.text.trim().split("\n").filter(l => l.length > 0);
                root.audioInputStreams = lines.map(l => {
                    const f = l.split("\t");
                    return { index: parseInt(f[0]) || 0, name: f[1] || "Unknown", vol: parseInt(f[2]) || 0, muted: f[3] === "1" };
                });
            }
    ...
    Timer {
        id: audioInputStreamsRefreshTimer
        interval: 400
        repeat: false
        onTriggered: { audioInputStreamsProbe.running = false; audioInputStreamsProbe.running = true; }
    }
    ...
    Component.onCompleted: {
        ...
        try {
            ...
            if (text && text.length > 0) {
                ...
                if (Array.isArray(saved))
                    root.cardItems = saved.map(function(d) {
                        return { appName: d.appName, summary: d.summary, body: d.body, time: d.time, notif: null };
                    });
            }
        } catch(e) {}
        ...
        try {
            ...
            if (text && text.length > 0) {
                const saved = JSON.parse(text);
                if (Array.isArray(saved)) root.clipHistory = saved;
            }
        } catch(e) {}
    }
    ...
    Process {
        ...
        running: false
        command: ["sh", "-c",
            "ls -t " + Quickshell.env("HOME") + "/Pictures/Screenshots/screenshot-*.png 2>/dev/null | head -60"]
        stdout: StdioCollector {
            onStreamFinished: {
                const lines = this.text.trim().split("\n").filter(s => s.length > 0);
    ...
    MusicPopup       { root: root }
    BatteryPopup     { root: root }
    ...
    IpcHandler {
        ...
        function toggle(): void {
            if (root.notificationsVisible) root.notificationsVisible = false;
            else root.openNotifications();
        }
        function open(): void  { root.openNotifications(); }
        function close(): void { root.notificationsVisible = false; }
        function dismiss(): void {
            if (root.cardItems.length > 0)
                root.removeFromCard(root.cardItems[root.cardItems.length - 1]);
        }
    ...
    IpcHandler {
        target: "tray"
        function toggle(): void {
            if (root.trayVisible) root.trayVisible = false;
            else root.openTray();
        }
        function open(): void  { root.openTray(); }
        function close(): void { root.trayVisible = false; }
    }
    ...
    IpcHandler {
        ...
        function clear(): void { root.clearClipHistory(); }
    }
    ...
    readonly property var _browserEntries: [
        "firefox", "chromium", "chromium-browser", "google-chrome",
        "brave-browser", "microsoft-edge", "opera", "vivaldi", "waterfox",
        "librewolf", "zen-browser", "thorium-browser"
    ]
    ...
    function refreshMusic() {
        ...
        for (let i = 0; i < players.length; i++) {
            ...
            if (root._browserEntries.indexOf(entry) !== -1) continue;
    ...
    function musicStop() {
        if (root.musicPlayer) root.musicPlayer.stop();
    }
    function openMusic() {
        root.closeAllCards();
