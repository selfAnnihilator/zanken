...
Item {
    ...
    readonly property string colorsPath: Quickshell.env("HOME") + "/.config/zanken/current/theme/colors.toml"
    ...
    Process {
        id: fontReader
        running: true
        command: ["cat", theme.monoFontPath]
        stdout: StdioCollector {
            onStreamFinished: {
                const f = this.text.trim();
                if (f) theme.mono = f;
            }
        }
    }
    ...
    IpcHandler {
        target: "font"
        function set(name: string): void { theme.mono = name; }
        function get(): string { return theme.mono; }
    }
