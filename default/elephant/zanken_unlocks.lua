--
-- Dynamic Zanken Unlocks Menu for Elephant/Walker
--
-- A "Default" entry restores the zanken-shipped Plymouth via
-- zanken-plymouth-reset. After that, every theme that has a preview-unlock.png
-- appears as a customised unlock; picking one runs zanken-plymouth-set-by-theme
-- <theme>. Both run in a floating terminal so sudo can prompt.
--
Name = "zankenunlocks"
NamePretty = "Zanken Unlocks"
...
function GetEntries()
  ...
  local user_themes_dir = home .. "/.config/zanken/themes"
  local zanken_path = os.getenv("OMARCHY_PATH") or ""
  local default_themes_dir = zanken_path .. "/themes"
  local default_preview = zanken_path .. "/default/plymouth/preview-unlock.png"
      ...
      if theme_name and not seen_themes[theme_name] then
        ...
        if file_exists(preview_path) then
          ...
          table.insert(entries, {
            ...
            PreviewType = "file",
            Actions = {
              activate = "zanken-launch-floating-terminal-with-presentation "
                .. shell_escape("zanken-plymouth-set-by-theme " .. shell_escape(theme_name)),
            },
          })
        end
      end
  ...
  local default_entry = {
    ...
    Actions = {
      activate = "zanken-launch-floating-terminal-with-presentation "
        .. shell_escape("zanken-plymouth-reset"),
    },
  }
